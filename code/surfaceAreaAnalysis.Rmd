...
# Combined the two columns in excel, keeping the paste0 code just in case
...
#   dplyr::filter(species %in% c("pc", "dl", "ps")) %>% 
...
saFragAov <- saClean %>% 
  select(timePoint, species,geno, ID, trial, surfaceArea) %>% 
  dplyr::filter(ID %in% 'frag') %>%
...
saFinalAov <- saComboAov %>% 
  dplyr::filter(timePoint %in% c("0","4")) %>%
  select(c('timePoint','species','geno','ID','trial','surfaceArea'))


saChangeAov <- dcast(saFinalAov, species+geno+ID+trial~timePoint) 
...
set.seed(999)
changePermSA <- adonis(formula = percentChange ~ species*geno*ID, data = saChangeAov, method = "euclidian", permutations = 9999, na.rm = TRUE)
changePermSA


set.seed(999)
changePairwise <- pairwise.adonis(saChangeAov[c(7)],factors=saChangeAov$species,
...
percentChangeFragPlotAovSA <- ggplot(plotFrags, aes(x = geno, y = percentChange))+
  geom_bar(stat = 'identity', position = 'dodge', fill = "darkblue")+
  ylim(-25, 100)+
  facet_wrap(trial~.)

percentChangeWildPlotAovSA <- ggplot(plotWilds, aes(x = geno, y = percentChange))+
 geom_bar(stat = 'identity', position = 'dodge', fill = 'navy')+
  ylim(-25, 100)


comboPlotAovSA <- percentChangeFragPlotAovSA + percentChangeWildPlotAovSA


ggsave("../figures/Fig3.png", plot = percentChangeFragPlotAov, height = 5, width = 9, units = 'in',dpi = 600)

```

#Making stylistic changes to the surface area plots

```{r, plotChange, include= TRUE}
percentChangeFragPlotAovSA <- percentChangeFragPlotAovSA + 
  labs(title = "Tissue Growth of Microfragments", 
       subtitle = "Graph of Percent Change in Tissue Surface Area by Species and Genotype", 
       x= "Coral Species and Genotype", 
       y= "Tissue Surface Area Percent Change")

percentChangeFragPlotAovSA <- percentChangeFragPlotAovSA + theme(
...
percentChangeWildPlotAovSA <- percentChangeWildPlotAovSA + 
  labs(title = "Tissue Growth of Wild Colonies", 
       subtitle = "Graph of Percent Change in Tissue Surface Area by Species and Genotype", 
       x= "Coral Species and Genotype", 
       y= "Tissue Surface Area Percent Change")

percentChangeWildPlotAovSA <- percentChangeWildPlotAovSA + theme(
  plot.title = element_text(color = "black", size = 20, 
                            face = "bold", hjust = 0.5),
  plot.subtitle = element_text(color = "black", size = 12,
                               hjust = 0.5),
...
comboPlotAovSA <- comboPlotAovSA + labs(title = "Tissue Growth ", 
                                        subtitle = "Percent Change of Tissue Surface Area")
