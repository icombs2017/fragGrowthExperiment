...
SApercentChange.1 <- ggplot(totalChange, aes(x = treatment, y = percentChange, fill = timepoint)) +
  geom_bar(stat = "identity", position = position_dodge())+
  facet_wrap(~id)+
...
SAgrowth.1 <- ggplot(data=totalSA, aes(x = treatment, y = SA, fill = timepoint)) +
  geom_bar(stat = "identity", position = position_dodge())+
  facet_wrap(~id)+
  scale_fill_hue(name="Time Point") +  
  labs(x = "Treatment", y = bquote("Surface Area" ~ (cm^2)),fill='Time Point') + # Set axis labels
...
```{r}
# meanDataset<- mean(totalChange[totalChange$id == "^DL"& totalChange$treatment=='frag'& totalChange$timepoint=='T3','percentChange'])
# 
# # plot(subset(meanDataset,select=c(id,percentChange)))
# 
# meanDataset
...
###DON'T BE MAD I DID THIS AGAIN, I COULDN'T FIGURE IT OUT
...
meanDataSet <- read_excel("../data/saDataset.xlsx", sheet = 7)
