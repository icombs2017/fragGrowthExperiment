...
totalChangeT3 <- dcast(totalT3, id+treatment~timepoint) 
...
# Going from long to wide so we can do the calculation
totalChangeT6 <- dcast(totalT6, id+treatment~timepoint) 
...
totalChange <- totalChange %>% 
  dplyr::select(id,treatment,T3percentChange,T6percentChange)
totalChangeMelt1 <- melt(totalChange, id = c(1,2))
...
totalChangeMelt <- totalChangeMelt1 %>% 
  ...
  dplyr::select(id,treatment,timepoint,percentChange)
...
logTotalLM <- lmer(logSA ~ treatment*timepoint + (1 | id), totalSA)
...
logChangeLM <- lmer(percentChange ~ treatment*timepoint + (1 | id), totalChange)
logChangeLManova <- anova(logChangeLM)
logChangeLManova
