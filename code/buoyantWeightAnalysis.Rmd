...
pacman::p_load("ggplot2", "googlesheets4", "dplyr", "officer","reshape2", "stringr", "dplyr", "flextable", "gridExtra", "ggpubr", "Rmisc", "rcompanion", "RColorBrewer", "vegan", "googledrive", "gdata", "readxl", "DescTools","patchwork")
...
bwClean <- bw %>% select("date", "timePoint", "species","geno", "trial", "ID", "dryWeight") %>% dplyr::filter(species %in% c("ps", "pc", "dl"))
...
bwFrags <- bwClean %>% dplyr::filter(ID %in% "frag") %>% select(c("date", "timePoint", "species", "geno", "trial", "ID", "dryWeight")) %>% group_by(timePoint, geno) %>% summarise_at(vars(dryWeight), mean)
...
bwFragPlot <- ggplot(bwFrags, aes(timePoint, dryWeight))+
 ...
 bwFinalPlot <- ggplot(firstAndLast, aes(timePoint, dryWeight))+
...
bwFinal <- bwClean %>% dplyr::filter(timePoint %in% c("0","4")) %>%
  select(c('date','timePoint','species','geno','trial','ID','dryWeight')) %>% 
  group_by("timePoint","geno")

length(bwFinal$timePoint)


bwFirst <- bwFinal %>% filter(timePoint %in% "0")
bwLast <- bwFinal %>%  filter(timePoint %in% "4")

bwChange <- bind_cols(bwFirst, bwLast$dryWeight)
bwChange$dryWeightInitial <- bwChange$dryWeight
bwChange$dryWeightFinal <- bwChange$...10
bwChange$percentChange <- ((bwChange$dryWeightFinal - bwChange$dryWeightInitial)/bwChange$dryWeightInitial)*100

bwChange <- bwChange %>% 
  ungroup() %>% 
  select(c("species","geno","trial","ID","percentChange"))

changeFrag <- bwChange %>% filter(ID %in% "frag")
changeWild <- bwChange %>% filter(ID %in% "wild")
