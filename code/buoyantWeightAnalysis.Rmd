...
bwFragAov <- bwClean %>% 
  select(timePoint, geno, ID, trial, dryWeight) %>% 
  dplyr::filter(ID %in% 'frag') %>%
...
bwFinalAov <- bwComboAov %>% 
  dplyr::filter(timePoint %in% c("0","4")) %>%
  select(c('timePoint','geno','ID','trial','dryWeight'))


bwChangeAov <- dcast(bwFinalAov, geno+ID+trial~timePoint) 
...
set.seed(999)
changePermBW <- adonis(formula = percentChange ~ geno*ID, data = bwChangeAov, method = "euclidian", permutations = 9999, na.rm = TRUE)
changePermBW
