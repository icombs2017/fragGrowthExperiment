...
pacman::p_load("ggplot2", "googlesheets4", "dplyr", "officer","reshape2", "stringr", "dplyr", "flextable", "gridExtra", "ggpubr", "Rmisc", "rcompanion", "RColorBrewer", "vegan", "googledrive", "gdata", "readxl", "DescTools","patchwork","reshape2", "lme4", "lmerTest")
...
bwTime <- bwClean %>% 
  dplyr::select("date", "timePoint", "species","geno", "trial", "ID", "dryWeight") %>% 
...
bwPlugs$avgPlug <- bwPlugs$dryWeight/5
meanPlug <- mean(bwPlugs$avgPlug)

#might need to multiply every genotype by n and average plugs in trial to make this work -_-
...
bwCleanFrags$correctedDryWeight <- bwCleanFrags$dryWeight - meanPlug #THIS NEEDS TO BE MULTIPLIED BY N OF EACH RACK -__-n AND EACH TRIAL??
...
bwClean <- rbind(bwCleanFrags, bwCleanWild)

totalFrag <- bwClean %>%
  select(timePoint, species, ID, geno, correctedDryWeight) %>%
  dplyr::filter(ID %in% 'frag') %>%
  dplyr::group_by(timePoint, species, ID, geno) %>%
...
bwFinal <- bwClean %>% 
  dplyr::filter(ID %in% c('frag', 'wild'))


plot <- ggplot(bwFinal, aes(timePoint, correctedDryWeight))+
  geom_point(aes(fill = 'black'), size = 2)+
  # geom_line(aes(color = geno), size = 4)+
...
bwFragAov <- bwClean %>% 
  select(timePoint, species, geno, ID, trial, dryWeight) %>% 
  dplyr::filter(ID %in% 'frag') %>%
  dplyr::group_by(timePoint,species,geno,ID,trial)
...
bwComboAov <- bind_rows(bwFragAov,bwWildAov)


bwFinalAov <- bwComboAov %>% 
  dplyr::filter(timePoint %in% c("0","4")) %>%
  select(c('timePoint','species','geno','ID','trial','dryWeight'))


bwChangeAov <- dcast(bwFinalAov, species+geno+ID+trial~timePoint) 

bwChangeAov <- dplyr::rename(bwChangeAov, "initialdryWeight" = "0", "finalDryWeight" = "4")
bwChangeAov
bwChangeAov$percentChange <- ((bwChangeAov$finalDryWeight-bwChangeAov$initialdryWeight)/bwChangeAov$initialdryWeight)*100


bwFragTestAov <- bwChangeAov %>% 
  select(species,geno, ID, percentChange) %>% 
  dplyr::filter(ID %in% 'frag') %>%
...
bwWildTestAov <- bwChangeAov %>% 
  dplyr::filter(ID %in% 'wild') %>% 
  select(species,geno, ID, percentChange)


bwComboAov <- left_join(bwFragTestAov, bwWildTestAov, by = c("geno"), copy = FALSE) %>% 
  dplyr::rename(fragpercentChange = percentChange.x) %>% 
  dplyr::rename(wildpercentChange = percentChange.y)

shapiro.test(bwFinalAov$dryWeight)
LeveneTest(bwFinalAov$dryWeight, group = bwFinalAov$ID)
...
set.seed(999)
changePermBW <- adonis(formula = percentChange ~ species*geno*ID, data = bwChangeAov, method = "euclidian", permutations = 9999, na.rm = TRUE)
changePermBW

mean(bwComboAov$fragpercentChange)
mean(bwComboAov$wildpercentChange)
